function [] = FIRtoIIR(projectName,subjectName,fileLength,fs,bit,Order,compression)
    ...
    for h=1:2
        if h==1
            sofaLoadFile = strcat('Audio/Projects/',projectName,'/SOFAFiles/SOFA_RAW/',subjectName,'/',subjectName,'_',int2str(fileLength),'order_fir_',fsFolder,'k_',int2str(bit),'bit.sofa');
            outputFileName = strcat('Audio/Projects/',projectName,'/SOFAFiles/SOFA_RAW/',subjectName,'/',subjectName,'_',int2str(fileLength),'order_biquads_',fsFolder,'k_',int2str(bit),'_bit.sofa');
        elseif h==2
            sofaLoadFile = strcat('Audio/Projects/',projectName,'/SOFAFiles/SOFA_HEQ/',subjectName,'/',subjectName,'_',int2str(fileLength),'order_fir_',fsFolder,'k_',int2str(bit),'bit.sofa');
            outputFileName = strcat('Audio/Projects/',projectName,'/SOFAFiles/SOFA_HEQ/',subjectName,'/',subjectName,'_',int2str(fileLength),'order_biquads_',fsFolder,'k_',int2str(bit),'_bit.sofa');
        end


        % Read IDT Lookup Table
        disp(strcat('Audio/Projects/',projectName,'/SOFAFiles/lookUpTables/',subjectName,'_ITD_Lookup_Table.txt'));
        fid = fopen(strcat('Audio/Projects/',projectName,'/SOFAFiles/lookUpTables/',subjectName,'_ITD_Lookup_Table.txt'),'r');
