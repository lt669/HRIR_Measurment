function   virtualAmbiStronixBinauralDecode_v1_2(ambisonicFile,...
                                                 HRTFFolder,...
                                                 saveAs,...
                                                 varargin)
%
...
% 03 / 01 / 2017: Furthered multi-decode strategy options implementation
...
%           dimension = '2d';
    ...
    for k = 1:length(varargin) % For each input option given...
       switch varargin{k}
           ...
           case [1, 2, 3, 4, 5] % 1st degree decode
               degree = 1;
               disp('User-selection: degree 1');
           case 2 % 2nd degree decode
               degree = 2;
               disp('User-selection: degree 2');
               degreeDimensionAmbiguity = 0;        % Remove ambiguity    
           case 3 % 3rd degree decode               % marker if degree 
               degree = 3;                          % or dimension is 
               disp('User-selection: degree 3');    % explicitely stated   
           case 4 % 4th degree decode
               degree = 4;
               disp('User-selection: degree 4');
               degreeDimensionAmbiguity = 0;        % ^^
           case 5 % 5th degree decode
               degree = 5;
               disp('User-selection: degree 5');
           ...
           case 'proj' % Projection decode
               decode = 'proj';
           case 'pseu-inv' % pseudo-inverse decode
               decode = 'pseu-inv';
...
%% Select Decode
    ...
    switch decode
        case 'proj'
            speakerSignals = (1/nChannels) * (C' * ambisonic')';
            
        case 'pseu-inv'
            speakerSignals = (pinv(C) * ambisonic')';
    end
...
%% Decode Virtual Loudspeaker Signals to Binaural
    ...
    audiowrite(sprintf('%s_%s_%s_degree%s_%s_%s.wav', saveAs, ...
                                                      decode, ...
                                                      configuration, ...
                                                      degree, ...
                                                      dimension, ...
