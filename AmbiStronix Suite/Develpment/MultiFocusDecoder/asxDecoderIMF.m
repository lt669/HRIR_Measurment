function output = asxDecoderIMF( ambisonicFileL,...
                                 ambisonicFileR,...
                                 HRTFFolder,...
                                 SaveAs,...
                                 varargin )
...
% Get Ambisonic data
    [ambisonicL, aFS] = audioread(ambisonicFileL);
    [ambisonicR, aFS] = audioread(ambisonicFileR);

    if not(size(ambisonicL) == size(ambisonicR))
        error('');
    end
    
% Process input options
    [decode, weighting, M, dim, norm, config, K, normTo1, supress] = ...
                             iasxDecoderOptions(ambisonicL, varargin{:}); % B-Format case
...
% Left
    lS(:, :, 1) = (D * ambisonicL(:, 1:K)')';
    
% Right
    lS(:, :, 2) = (D * ambisonicR(:, 1:K)')';    
