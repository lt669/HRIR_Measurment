function output = asxDecoderMF( ambisonicFile,... 
                                HRTFFolder,...
                                SaveAs,...
                                varargin )
...
% 09 / 05 / 2017: Adapted for single chennel output
...
% Process input options
    [decode, weighting, M, dim, norm, config, K, normTo1, supress] = ...
                             iasxDecoderOptions(ambisonic, varargin{:}); % B-Format case
...
%% Calculate Virtual Loudspeakerpeaker Signals
    ...
    rot_zyx = asxRotMatrix_zyx(1);

% Left
    [DL, ~] = iasxD(phi, theta, decode, M, dim, norm);
    DL = iasxDWeighting(DL, weighting, M);
    ambisonicL = asx3DRotate(ambisonic(:, 1:K)', rot_zyx, 5*pi/180, 0, 0);
    lS(:, :, 1) = (DL * ambisonicL)';
    
% Right
    [DR, ~] = iasxD(phi, theta, decode, M, dim, norm);
    DR = iasxDWeighting(DR, weighting, M);
    ambisonicR = asx3DRotate(ambisonic(:, 1:K)', rot_zyx, -5*pi/180, 0, 0);
    lS(:, :, 2) = (DR * ambisonicR)';    
    
%% Decode Virtual Loudspeaker Signals to Binaural
    
    [output, FS] = iasxA2BMF (lS,...
                              HRTFFolder,...
                              phiDeg,...
                              thetaDeg,...
                              aFS,...
